...
$sql = "SELECT p.*, pas.Nama, pas.Nomor_Rekam_Medis, rm.Tekanan_Darah, rm.Tinggi_Badan, 
        ...
        LEFT JOIN Resep r ON pe.ID_Pemeriksaan = r.ID_Pemeriksaan
        WHERE p.ID_Pendaftaran = ? AND p.Status = 'Diperiksa'";
...
<head>
    ...
    <title>Lanjutkan Pemeriksaan - Poliklinik X</title>
    <style>
        :root {
        ...
        <form action="process_examination.php" method="POST" class="card">
            ...
            <div class="form-group">
                ...
                <textarea id="diagnosa" name="diagnosa" required 
                          placeholder="Masukkan hasil diagnosa pasien..."><?php echo htmlspecialchars($patient['Diagnosa'] ?? ''); ?></textarea>
            </div>

            <div class="form-group">
                <label for="resep">Resep Obat</label>
                <textarea id="resep" name="resep" required
                          placeholder="Masukkan resep obat untuk pasien..."><?php echo htmlspecialchars($patient['Resep_Obat'] ?? ''); ?></textarea>
